...
**Tier 2 done (code + additive rules):** new uploads now write to `library/{orgId}/{itemId}/...` and `vendors/{orgId}/{vendorId}/...` (`organizationId` threaded through the upload helpers in `db.ts`, the two image-mirror helpers, and all callers), with org-checked rule blocks added. The flat legacy blocks (`library/{p=**}`, `vendors/{p=**}`) are intentionally retained so existing files display and stored-path deletes work. **Caveat:** Storage rules grant if ANY matching block allows, so while the permissive flat blocks exist they also cover the new org-scoped paths — cross-org isolation on library/vendors is only fully enforced once the flat blocks are removed. **Remaining (Tier 2 cleanup):** migrate/re-upload the few existing flat-path files, then delete the two flat rule blocks.
