...
export async function getWeatherForLocation(locationQuery?: string) {
  try {
    ...
    if (!apiKey) {
      console.warn("[Weather API] WEATHER_API_KEY is not defined in .env.local");
      return { success: false, error: "WEATHER_API_KEY is missing from environment variables." };
    }

    // Default fallback to New York ZIP code if user location is not set yet
    const query = locationQuery && locationQuery.trim() !== "" ? locationQuery.trim() : "10001";

    console.log(`[Weather API] Fetching weather from weatherapi.com for query: ${query}`);
    ...
    if (!res.ok) {
      ...
      return { success: false, error: `Weather service responded with status: ${res.status}` };
    }
