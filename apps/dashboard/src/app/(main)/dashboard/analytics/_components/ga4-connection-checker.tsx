...
import { testGA4Connection, GA4ConnectionResult } from "@/server/analytics-actions";
          ...
          <div>
            ...
            <p className="text-muted-foreground text-xs mt-1.5 leading-normal max-w-xl">
              ...
              {!loading && !result?.success && result?.configMissing && (
                <>
                  Credentials missing. Add <code className="bg-muted px-1.5 py-0.5 rounded text-[11px] font-mono">GA_PROPERTY_ID</code>,{" "}
                  ...
                  <code className="bg-muted px-1.5 py-0.5 rounded text-[11px] font-mono">GA_REFRESH_TOKEN</code> inside your <code className="font-semibold text-foreground">.env.local</code>.
                </>
              )}

              {!loading && !result?.success && !result?.configMissing && (
                <>
                  Connection failed: <span className="text-destructive font-mono text-[11px]">{result?.error}</span>. Make sure your Google Account has read access to the specified Property ID.
                </>
              )}
            </p>
          </div>
