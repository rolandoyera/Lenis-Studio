  ...
  if (!result.success || !result.data) {
    return (
      <div className="flex min-h-[200px] flex-col items-center justify-center gap-2 rounded-xl border border-dashed bg-card p-8 text-center text-muted-foreground ring-1 ring-foreground/10">
        <AnalyticsErrorToast error={result.error} title="Acquisition Error" />
        <span className="font-semibold text-foreground text-sm">Failed to load acquisition metrics</span>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Card className="gap-2">
        <CardHeader>
          <CardTitle className="font-normal">Channels</CardTitle>
          <CardAction>
          ...
          <Table className="[&_td:first-child]:pl-4 [&_td:last-child]:pr-4 [&_th:first-child]:pl-4 [&_th:last-child]:pr-4">
            <TableHeader className="[&_tr]:border-border/50">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="h-8 w-20 text-right font-normal">Sessions</TableHead>
                <TableHead className="h-8 w-20 text-right font-normal">Users</TableHead>
                <TableHead className="h-8 w-24 text-right font-normal">Engagement</TableHead>
              ...
              {channels.length === 0 ? (
                ...
                channels.map((row) => (
                  <TableRow className="hover:bg-transparent" key={row.channel}>
                    ...
                    <TableCell className="text-right text-muted-foreground tabular-nums">{row.users}</TableCell>
      ...
      <Card className="gap-2">
        <CardHeader>
          <CardTitle className="font-normal">Source / Medium</CardTitle>
          <CardAction>
            <Ellipsis className="size-4" />
          </CardAction>
        </CardHeader>
            ...
            <TableBody className="[&_tr]:border-border/50">
              {sourceMedium.length === 0 ? (
                ...
                sourceMedium.map((row) => (
                  <TableRow className="hover:bg-transparent" key={`${row.source}/${row.medium}`}>
                    <TableCell className="max-w-0 truncate py-4 font-medium">{row.source}</TableCell>
