...
interface VendorDetailItemsProps {
  items: LibraryItem[];
}

export function VendorDetailItems({ items }: VendorDetailItemsProps) {
  return (
          ...
          <div className="divide-y divide-border/40">
            {items.map((item) => (
              <div key={item.itemId} className="flex items-center gap-3 p-3">
                {/* Thumbnail */}
                <div className="flex size-12 shrink-0 items-center justify-center overflow-hidden rounded border border-border/50 bg-background/50">
                  {item.coverImageUrl ? (
